...
| I.31 | Draw a parallel through a point | C | ✅ | 7 | Point.appear, Line.straightEdgeConnect, Sector.sweep | ✓ | **Q.E.F. construction** (`red_highlight`). Random D on BC, join AD, copy ∠ADC to ∠DAE at A (@I.23 — shown as the `similar` ray AE, not the full ten-circle walk), produce AF; EAF∥BC by @I.27 (alternate ∠EAD=∠ADC). Constructed parallel E–A–F drawn as AE+AF; **zero-color `EF;connect;E,F` is the unified hover target**, with aliases **DAE→EAD** (prose uses both spellings for the A-angle) and **EAF→EF**. 2 markers (∠EAD@A, ∠ADC@D). D deferred. Guide (uniqueness via Post.5; works in hyperbolic too) tokenized to canvas_0. **canvas_1 = a 9-slide "six circles + a line" construction walk** in the "Construction steps" section (Nelson-requested), built EXPLICITLY (no compassTransfer macro — Nelson's call), **one slide per circle stating why it is drawn** (+ framing, meet-at-F, count). Shows exactly Joyce's six circles + one line + the construction points P/T/G/F (no markers/chord/equal-sides). Copies ∠ADC onto AD at A. The DA-side transfer collapses (A already on ray DA), so per Joyce the six = the four I.1/I.2 chord-transfer circles (K1,K2 equilateral on DP → apex T; K3,K4 lay-off) + the two result circles (Acirc ctr A r=side, Gcirc ctr D r=chord); one line = AF. Joining **D–P** keeps all six distinct (K1 = ctr D r=DA doubles as the equal-sides circle that finds P, so none is drawn just for P). Captions use Joyce's enumeration verbatim ("four circles … as per I.1 and I.2", "two more circles", "these last two circles meet at F"). **F = `first` confirmed correct** (Nelson's screenshot: AF parallel to DC, on the left). T = `first` of K1∩K2 (lower apex) still worth a glance while stepping slides 2–5, but it only positions the K4 scaffolding, not the result. **Resting/static canvas cleaned via `initiallyHidden: [P,T,G,K1,K2,K3,K4]`** → at rest it shows only the slide-9 demo (the angle, the two result circles, AF, and the F/F2 comparison); the six-circle walk is revealed only by stepping. Also carries a **second intersection F2/AF2 (red)** + a "choice of F matters" demo slide and an alternate-angle illustrative slide (∠FAD vs ∠F2AD, @I.27) showing why only F gives the parallel. K4 (ctr T, r=side+chord) is large and runs off-canvas by design — present-mode frames it. Markers → #91 rework |

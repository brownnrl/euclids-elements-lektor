...
| I.31 | Draw a parallel through a point | C | ✅ | 7 | Point.appear, Line.straightEdgeConnect, Sector.sweep | ✓ | **Q.E.F. construction** (`red_highlight`). Random D on BC, join AD, copy ∠ADC to ∠DAE at A (@I.23 — shown as the `similar` ray AE, not the full ten-circle walk), produce AF; EAF∥BC by @I.27 (alternate ∠EAD=∠ADC). Constructed parallel E–A–F drawn as AE+AF; **zero-color `EF;connect;E,F` is the unified hover target**, with aliases **DAE→EAD** (prose uses both spellings for the A-angle) and **EAF→EF**. 2 markers (∠EAD@A, ∠ADC@D). D deferred. Guide (uniqueness via Post.5; works in hyperbolic too) tokenized to canvas_0. **canvas_1 = a 7-slide "six circles + a line" construction walk** in the "Construction steps" section (Nelson-requested): copies ∠ADC onto AD at A. Accounting matches Joyce — the DA-side transfer collapses (A already on ray DA ⟹ G≡D, 4 scaffolding circles saved), leaving Acirc + the chord-transfer of AP to D (compassTransfer, 4 kept gK* + Gcirc) = 6, plus Dcirc (Apollonius equal-sides, uncounted like I.23's Ccirc) and line AE. **⚠ needs visual check: (1) E = `bichord;Acirc,Gcirc` `first` — should land the LEFT intersection so AE is horizontal/parallel to DC; flip to `last` if AE points toward P. (2) compassTransfer `side:1` scaffolding direction.** Markers → #91 rework |

...
| I.22 | Triangle from three given lines | C | ✅ | 12 | Point.appear, Line.straightEdgeConnect, Circle.compass, Polygon.outline | — | 2nd Q.E.F. construction deck (after I.1; generalizes it). Lay off A/B/C along ray DE (cutoffs DF/FG/GH), strike circles DKL (ctr F, r=FD) + KLH (ctr G, r=GH), apex K = their bichord intersection, join KF/KG → △FGK with sides A,B,C. Given lines A/B/C are draggable slider inputs (stay visible from S1); deferDraggables D/E. Added KF/KG as black lines (coincide with FGK edges). aliases KFG→FGK, FK→KF, GK→KG, FD→DF, LKH→KLH. **Introduces the editorial-footnotes convention** (see conventions.md): two original Joyce typos kept verbatim + flagged via superscript → footnotes at page end (proof "B+C>B" should be A; guide "B,C,and D" should be A,B,C). No markers (construction) |

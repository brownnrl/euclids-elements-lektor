...
| I.20 | Triangle inequality | T | ✅ | 9 | Line.straightEdgeExtend, Point.appear, Line.straightEdgeConnect, Sector.sweep, Polygon.outline | ⚠ | BA produced to D (DA=AC), DC joined; isosceles △ACD (I.5) ⟹ ∠BCD>∠BDC ⟹ DB>BC (I.19). 3 markers (∠ACD's marker named DCA since "ACD" is the isosceles polygon; prose `{ACD\|DCA}`). Guide has a big 2nd canvas — Heron's reflection / minimum-distance proof — its refs now bound to canvas_1 (added BF/FB'/AE/EP/EB' hover targets + AEC/BED incidence/reflection markers; aliases BE/BP/B'P). Markers → #91 rework |

...
│
├── assets/                         # served verbatim under build/
...
│           └── bookII/ … bookXIII/ # not yet authored
│
├── scripts/
│   └── deploy-preview.sh           # build + force-push to euclids-elements.org/lektor-<branch>
│
...
- **The build directory is meant to be ephemeral.** Cloudflare Workers serves it; the local `build/` is just for spot-checks.
